...
class MassMailController(http.Controller):
    ...
    def _check_unsubscribe_token(self, mailing_id, res_id, email, token):
    ...
    def mailing(self, mailing_id, email=None, res_id=None, token="", **post):
        ...
        if mailing.exists():
            ...
            if not self._check_unsubscribe_token(mailing_id, res_id, email, token):
            ...
            if mailing.mailing_model_real == 'mail.mass_mailing.contact':
                ...
                blacklist_rec._message_log(_("""Requested blacklisting via unsubscribe link."""))
    ...
    def blacklist_add(self, mailing_id, res_id, email, token):
        ...
        if email:
            blacklist_rec = request.env['mail.blacklist'].sudo()._add(email)
            blacklist_rec._message_log(_("""Requested blacklisting via unsubscription page."""))
            return True
        return 'error'
    ...
    def blacklist_remove(self, mailing_id, res_id, email, token):
        ...
        if email:
            ...
            blacklist_rec._message_log(_("""Requested de-blacklisting via unsubscription page."""))
            return True
        return 'error'
